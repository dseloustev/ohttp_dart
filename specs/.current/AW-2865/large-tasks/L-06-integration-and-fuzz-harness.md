...
The integration test should also assert the `OhttpClient` surface that earlier tasks introduced — namely, the typed exception types from L-02 surface on the matching failure paths (gateway 4xx/5xx, timeout, AEAD failure), the KeyConfig cache from L-03 short-circuits subsequent calls within TTL, and the observer events from L-04 fire with the expected fields.
...
- **`parseResponse(randomBytes)` never throws anything other than `FormatException`** (coordinates with the BHTTP parser hardening from L-03 that converts `RangeError` to `FormatException`).
- **`OhttpKeyConfig.parse(randomBytes)` never throws anything other than the typed library exception(s)** (coordinates with the typed-exception hierarchy from L-02 and the multi-suite parsing fix).
...
- Integration test asserts the L-02 typed exception types fire on the expected failure paths, the L-03 KeyConfig cache short-circuits within TTL, and the L-04 observer events fire with the documented fields.
...
- This task should land last because it asserts behaviour established by L-01 (sendDirect surface), L-02 (typed exceptions), L-03 (timeouts, caching, parser caps), and L-04 (observer events).
- Source merged tasks (in `../merged-tasks/`): M-12 (integration + fuzz).
