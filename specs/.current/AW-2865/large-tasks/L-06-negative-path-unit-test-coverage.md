...
After the L-01 restructure, the legacy `test/ohttp_client_test.dart` no longer exists (`OhttpClient` is removed). Gateway-status negative-path tests against the typed exception hierarchy land in `test/adapters/http_adapter_test.dart` and `test/ohttp_session_test.dart` (introduced by L-01) — those tests already get most of their hierarchy assertions from L-03; this task focuses on **primitive-layer** negative paths that don't depend on `OhttpSession` or `HttpClientTransport`.

Depends on the BHTTP parser hardening from L-04 (truncation tests assert `FormatException` rather than `RangeError`) and the typed exception hierarchy from L-03 (tampered-ciphertext test asserts `OhttpDecryptionException`).

## Technical Details

### Part A — HPKE coverage (`test/hpke_test.dart`)

**1. Sequence-number overflow in `HpkeSenderContext.seal`**

`HpkeSenderContext.seal` computes the per-call nonce by XOR-ing `_seq` into `baseNonce`. RFC 9180 §5.2 caps the sequence number at `2^(8*Nn) - 1`; on overflow, the implementation must refuse to seal. AES-GCM nonce reuse after overflow is catastrophic. The existing `seal seq=0` and `seal seq=1` tests in the file end before any overflow case; no overflow test exists.
...
**3. `HpkeSenderContext.export` with invalid length**
...
**4. QUIC varint 8-byte boundary round-trip**
...
**5. `parseResponse` truncated body**

`lib/src/bhttp.dart` `parseResponse` performs `data.sublist(...)` calls that throw `FormatException` once the parser is hardened in L-04.
...
Assert that the exception thrown is `FormatException` with a useful message. If L-04 has not yet landed, pin the current `RangeError` behaviour and update when L-04 lands.
...
Test name references RFC 9458 §4.3 and §4.4. This helper doubles as documentation of the response-channel contract — useful for anyone implementing a Dio adapter or a custom gateway emulator.

**8. AEAD authentication failure**

`ohttpDecapsulate` decrypts the response with `aesGcm.decrypt(...)`. After L-03 lands, decapsulation surfaces `OhttpDecryptionException` on auth failure.

Add a test that produces a valid encap, derives the response secret as in test 7, encrypts a response, then flips one byte in the ciphertext before calling `ohttpDecapsulate`. Assert that decap throws `OhttpDecryptionException` (or the underlying cryptography type if L-03 has not yet landed; pin and update). Test name references RFC 9458 §4.4 and AEAD authentication semantics.
...
- Round-trip with a payload at exactly the configured response-size cap (succeed) and one byte above (expect rejection from L-04's cap, which lives in `OhttpSession.send`, not in `ohttpDecapsulate`).

Note: the cap test at "one byte above" tests the orchestration layer, not the primitive. If exercising it requires `OhttpSession`, place that test under `test/ohttp_session_test.dart` instead and leave only the primitive-layer edge cases in `test/ohttp_test.dart`.

If L-04's size caps have not landed, pin current behaviour and update when the cap lands. Tests reference RFC 9458 §4.3 / §4.4 where applicable.

### Part D — Interop-hazard comments in `lib/src/ohttp.dart`

**10. Empty-AAD maintenance-hazard comment**

Current line ~148 of `ohttp.dart`: `// Seal with empty AAD (per reference Go implementation, not RFC header)`. Expand the comment block to:
...
**11. Plain-HKDF maintenance-hazard comment**

Lines ~192–207 of `ohttp.dart` are the plain-HKDF block in `ohttpDecapsulate`. The existing comment (`// prk = HKDF-Extract(salt, secret) — plain HKDF, not labeled`) hints at the issue but does not state the consequences. Expand to:
...
**HPKE:**
- Sequence-number overflow test added; exercises the boundary at `_seq == 2^(8*Nn) - 1` and the overflow case; asserts a library-owned exception. If the current code lacks the cap, the implementation fix is included.
...
- `export` invalid-length tests added; cover `length == 0`, `length == Nh`, and `length > 2^32 * Nh`; assert a library-owned exception on the over-cap case. If the implementation lacks the cap, the implementation fix is included.

**BHTTP:**
- Varint 8-byte boundary round-trip test added; covers the three boundary values; asserts byte-level encoding equality and decoded-value equality; references RFC 9000 §16.
- Truncated-body tests added; cover header-name, header-value, and content truncation positions; assert `FormatException` with a useful message (assumes L-04 has landed).
...
- New tampered-ciphertext test asserts decap throws `OhttpDecryptionException` (or underlying type if L-03 has not yet landed).
- New edge-case tests cover empty `binaryRequest`, empty response payload, and (in `test/ohttp_session_test.dart`) payload at cap, payload above cap.

**Comments:**
- Empty-AAD comment block in `ohttp.dart` is expanded to at least the four points listed; includes a URL or commit reference to the Go interop implementation.
- Plain-HKDF comment block in `ohttp.dart` is expanded to at least the four points listed.
...
- Tests: pure unit tests in the three existing test files (`hpke_test.dart`, `bhttp_test.dart`, `ohttp_test.dart`); the OHTTP round-trip helper doubles as documentation of the response-channel contract.
- The comment expansions are self-contained code-doc edits; no behavioural change.
- Depends on L-01 (the new test files `key_config_cache_test.dart`, `ohttp_session_test.dart`, `adapters/http_adapter_test.dart` exist; legacy `ohttp_client_test.dart` is gone).
- Depends on L-04 (parser hardening) and L-03 (typed exception hierarchy). If either has not landed, pin current behaviour and update when the upstream task lands.
